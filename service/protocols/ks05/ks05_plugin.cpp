...
class Ks05Protocol : public PsiProtocol {
public:
    std::string name() const override { return "ks05_t_mpsi"; }
    ...
    std::vector<std::string> run(const ProtocolContext& ctx,
        ...
        if (ctx.is_leader) {
            return runLeader(inputs, ctx.threshold, setSize, ctx.member_ids);
        } else {
...
REGISTER_PROTOCOL(Ks05Protocol)
