...
namespace osuCrypto
{
    ...
    struct ArrayIterator
    {
        ...
        {
            if (mCur > mEnd)
                throw std::runtime_error("iter went past end. " LOCATION);
            if (mCur && mCur < mBegin - 1)
                throw std::runtime_error("iter went past begin. " LOCATION);
        }
        T *mBegin, *mCur, *mEnd;
        ...
        T &operator*()
        {
            if (mCur >= mEnd || mCur < mBegin)
                throw std::runtime_error("deref past begin or end. " LOCATION);
            return *mCur;
        }
        ...
        T &operator[](i64 i)
        {
            if (mCur + i >= mEnd || mCur + i < mBegin)
                throw std::runtime_error("idx went past begin or end. " LOCATION);
            return mCur[i];
        }
    ...
    public:
        ...
        ArrayView(const ArrayView &av) : mData(av.mData),
                                         mSize(av.mSize),
                                         mOwner(false)
        {
        }

        ArrayView(ArrayView &&av) : mData(av.mData),
                                    mSize(av.mSize),
                                    mOwner(av.mOwner)
        {
            av.mData = nullptr;
        ...
        ArrayView(u64 size) : mData(new T[size]),
                              mSize(size),
                              mOwner(true)
        {
        }

        ArrayView(T *data, u64 size, bool owner = false) : mData(data),
                                                           mSize(size),
                                                           mOwner(owner)
        {
        }
        ...
        template <class Iter>
        ArrayView(Iter start, Iter end, typename Iter::iterator_category *p = 0) : mData(&*start),
                                                                                   mSize(end - start),
                                                                                   mOwner(false)
        {
        ...
        ArrayView(T *begin, T *end, bool owner) : mData(begin),
                                                  mSize(end - begin),
                                                  mOwner(owner)
        {
        }
        ...
        ArrayView(const C<T, Args...> &cont, typename C<T, Args...>::value_type *p = 0) : mData(((C<T, Args...> &)cont).data()),
