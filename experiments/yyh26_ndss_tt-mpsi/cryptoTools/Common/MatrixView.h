...
namespace osuCrypto
{
    ...
    public:
        ...
        MatrixView(const MatrixView &av) : mData(av.mData),
                                           mSize(av.mSize),
                                           mOwner(false)
        {
        }

        MatrixView(MatrixView &&av) : mData(av.mData),
                                      mSize(av.mSize),
                                      mOwner(av.mOwner)
        {
            av.mData = nullptr;
        ...
        MatrixView(u64 rowSize, u64 columnSize) : mData(new T[rowSize * columnSize]()),
                                                  mSize({rowSize, columnSize}),
                                                  mOwner(true)
        {
        }

        MatrixView(T *data, u64 rowSize, u64 columnSize, bool owner) : mData(data),
                                                                       mSize({rowSize, columnSize}),
                                                                       mOwner(owner)
        {
        }

        MatrixView(T *start, T *end, u64 numColumns) : mData(&*start),
        ...
        template <class Iter>
        MatrixView(Iter start, Iter end, u64 numColumns, typename Iter::iterator_category *p = 0) : mData(&*start),
                                                                                                    mSize({(end - start) / numColumns, numColumns}),
                                                                                                    mOwner(false)
        {
        ...
        MatrixView(const C<T, Args...> &cont, u64 numColumns, typename C<T, Args...>::value_type *p = 0) : mData(&*((C<T, Args...> &)cont).begin()),
