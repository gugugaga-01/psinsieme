    ...
    std::ostream &operator<<(std::ostream &out, const Timer &timer)
    {
        if (timer.mTimes.size())
        {
            ...
            while (++iter != timer.mTimes.end())
            {
                ...
                out << "  " << std::chrono::duration_cast<std::chrono::milliseconds>(iter->first - timer.mStart).count() << "  " << std::chrono::duration_cast<std::chrono::microseconds>(iter->first - prev->first).count() << std::endl;
