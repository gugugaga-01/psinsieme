...
namespace osuCrypto
{
    ...
    {
        return ((mCurve->mIsPrimeField ? mCurve->mEccpParams.bitCount : mCurve->mEcc2mParams.bitCount) + 7) / 8 + 1;
    }
    ...
    EccNumber::EccNumber(const EccNumber &num)
        : mVal(nullptr), mCurve(num.mCurve)
    {
        init();
    ...
    EccNumber::EccNumber(EccNumber &&num)
        : mVal(num.mVal), mCurve(num.mCurve)
    {
        num.mVal = nullptr;
    }
    ...
    {
        return ((mCurve->mIsPrimeField ? mCurve->mEccpParams.bitCount : mCurve->mEcc2mParams.bitCount) + 7) / 8;
    }
