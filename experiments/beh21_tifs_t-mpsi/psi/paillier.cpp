...
Ciphertext enc(const Plaintext &m, const PubKey &pk)
{
    ...
    // Validate plaintext range: should be in (-n/2, n/2]
    if (m > pk.n / 2 || m <= -(pk.n / 2))
        throw std::invalid_argument("plaintext out of valid range (-n/2, n/2]");

    // Normalize negative plaintexts to positive representation
