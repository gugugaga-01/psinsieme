...
namespace osuCrypto
{
    ...
    public:
		...
		void recvSSTableBased(u64 IdxP, binSet& bins, std::vector<block>& plaintexts, const std::vector<Channel*>& chls);
		void recvSSTableBased(u64 IdxTheirParty, binSet& bins, std::vector<block>& plaintexts, const std::vector<Channel*>& chls, const std::vector<u64> FourModulo, const u64 index);
