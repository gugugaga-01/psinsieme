...
namespace osuCrypto
{
    ...
    //     : mSize(other.mSize), mOwner(false)
    // {
    //     // Allocate memory
    //     mData = new T[mSize[0] * mSize[1]];

    //     // Perform deep copy
    //     std::memcpy(mData, other.mData, sizeof(T) * mSize[0] * mSize[1]);
    // }
        ...
        const MatrixView& deepcopy(const MatrixView& other)
    {
        if (this != &other) // Avoid self-assignment
        {
            // Release existing data
            if (mOwner && mData != nullptr)
            {
                delete[] mData;
            }

            // Copy size and ownership
            mSize = other.mSize;
            mOwner = true;
